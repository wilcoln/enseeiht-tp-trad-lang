...
let pexpr_value = (value_of_expr pexpr env) in 
    ...
    | (ErrorValue _) as result -> result
    | _ -> 
    (let fexpr_value = (value_of_expr fexpr env) in 
      match fexpr_value with 
          | FrozenValue(FunctionNode(x, expr_retour), env_def) -> (value_of_expr expr_retour (((x, pexpr_value)::env_def)))
          | _ -> (ErrorValue TypeMismatchError) 
    )
(* A traiter*)
(* ========================================================*)
